...
export default async (req, res) => {
  try {
    ...
    SELECT 1 FROM Director WHERE username = '${req.query.username}';
    `;
    ...
    db.query(checkDirectorQuery, (err, data) => {
      ...
      if (!data.rows[0]) {
        return res.status(404).json({ resultMessage: "The director with the given username can not be found." })
      }
